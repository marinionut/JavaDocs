...
public class Exercise1 {
    ...
    public List<String> iteratingOverEntries(){
        ...
        for(Map.Entry<String, String> name : countries.entrySet()) {
            if(name.getKey().contains("Q") || name.getKey().contains("q")) {
                seekingCountries.add( name.getKey().toLowerCase());
            }
        }
        return seekingCountries;
    }
